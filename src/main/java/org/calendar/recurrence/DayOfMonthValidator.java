...
public class DayOfMonthValidator implements Validator {
    ...
    public boolean isValid(LocalDateTime date) {
        ...
        return ordinalDays.stream().anyMatch(
                reversedOrder -> dayOfMonth == lastDayOfMonth.plusDays(reversedOrder).getDayOfMonth());
    }
    ...
    public LocalDateTime nextClosestValidDate(LocalDateTime currentDate) {
        ...
        if (validDays != null) {
            ...
            Optional<Integer> reversedDay =
                    ordinalDays.stream().filter(reversed -> (lastDayOfMonth + reversed) > curDayOfMonth)
                            .findFirst();
